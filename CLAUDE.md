...
对外形态：**HTTP Agent 服务** + **Web 客户端**（暂不提供 Tauri / Electron 桌面壳）。
...
- **Web**：`apps/web`（Vite + React + antd + Monaco Editor，经 `/api` 访问服务）
- **Markdown**：自研 `MarkdownView`（代码块用 Monaco 渲染）
- **运行日志**：Web 侧 `RunLogPanel` 聚合 SSE 事件输出
- **契约**：`packages/shared`（HTTP 路径、事件 DTO）
- **冒烟**：`apps/smoke-cli`
...
npm run smoke -- "列出当前目录文件"
npm run typecheck
```

依赖安装优先 `--ignore-scripts`（与 Pi 供应链建议一致）。

## Pi / HTTP 集成约定

- 会话创建：`createAgentSession` + `ModelRuntime` + `SessionManager`（仅在 server / runtime）
- 事件：Pi → `AgentEventDto` → SSE（`event: session`）→ Web `EventSource`
- 前端只通过 `apps/web/src/lib/api.ts`（fetch + EventSource），禁止直接访问 Pi
- 扩展点挂在 `runtime` / server，不进 UI 层
- 首期无鉴权（本机/内网）；不要重新引入 Tauri / Electron
...
4. **可验证**：改完后尽量通过 `typecheck` / 相关测试；无法运行时说明原因与风险点。
...
- 公共 API、数据模型、HTTP 载荷必须有明确类型，放在 `@aircode/shared`。
...
- HTTP 路径与 SSE 事件名见 `HttpPaths` / `SseEventName`。

### 错误处理

- HTTP 边界返回 `{ error: string }` + 合适 status；前端映射为 `{ ok: false, error }`。
...
- 仅使用函数组件 + Hooks。
- UI 优先使用 antd；代码展示用 `CodeViewer`（Monaco）；Markdown 用自研 `MarkdownView`。
- 运行过程输出写入 `RunLogPanel`，由 SSE 事件驱动。
- 所有 Agent 调用经过 `src/lib/api.ts`，禁止组件内散落 `fetch`。
- 不要默认添加 `useMemo` / `useCallback`。

### Server / Node

- Agent 业务只在 `packages/runtime`；`apps/server` 负责 HTTP 编排与 SSE。
- 会话与子进程生命周期必须可清理（进程退出时 `disposeAll`）。

### 测试与质量

- 纯逻辑优先单测；契约变更时同步更新 shared、server 与 web client。
